...
<?php $__env->startSection('title', 'Laporan Cashflow - ' . config('app.name')); ?>
<?php $__env->startSection('page-title', 'Laporan Cashflow'); ?>
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex space-x-2">
                <a href="<?php echo e(route('laporan.export.pdf', ['type' => 'cashflow', 'start_date' => request('start_date', now()->startOfMonth()->format('Y-m-d')), 'end_date' => request('end_date', now()->endOfMonth()->format('Y-m-d')), 'group_by' => request('group_by', $groupBy ?? 'day')])); ?>" 
               target="_blank"
               class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm inline-flex items-center">
                ...
                <a href="<?php echo e(route('laporan.export.excel', ['type' => 'cashflow', 'start_date' => request('start_date', now()->startOfMonth()->format('Y-m-d')), 'end_date' => request('end_date', now()->endOfMonth()->format('Y-m-d')), 'group_by' => request('group_by', $groupBy ?? 'day')])); ?>" 
    ...
    <div class="bg-white rounded-xl shadow-sm border p-4">
        <form method="GET" action="<?php echo e(route('laporan.cashflow')); ?>" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="group_by" class="w-full px-4 py-2.5 text-sm rounded-lg border border-gray-300 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="day" <?php if(request('group_by', $groupBy ?? 'day') === 'day'): echo 'selected'; endif; ?>>Per Hari</option>
                        <option value="month" <?php if(request('group_by', $groupBy ?? 'day') === 'month'): echo 'selected'; endif; ?>>Per Bulan</option>
                        <option value="year" <?php if(request('group_by', $groupBy ?? 'day') === 'year'): echo 'selected'; endif; ?>>Per Tahun</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3">
                ...
                <a href="<?php echo e(route('laporan.cashflow')); ?>" class="w-full sm:w-40 px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium transition-colors text-center inline-flex items-center justify-center">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div class="bg-white rounded-xl shadow-sm border p-6">
            ...
            <p class="text-2xl font-bold text-green-600"><?php echo e(format_rupiah($pemasukan)); ?></p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border p-6">
            <p class="text-sm text-gray-600">Total Pengeluaran</p>
            <p class="text-2xl font-bold text-red-600"><?php echo e(format_rupiah($pengeluaran)); ?></p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border p-6">
            <p class="text-sm text-gray-600">Saldo Akhir</p>
            <p class="text-2xl font-bold text-blue-600"><?php echo e(format_rupiah($saldo)); ?></p>
        </div>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <?php echo e(\Carbon\Carbon::parse($trans['tanggal'])->format('d/m/Y')); ?>

                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">
                            <?php echo e($trans['keterangan']); ?>

                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm <?php echo e($trans['pemasukan'] > 0 ? 'text-green-600 font-semibold' : 'text-gray-400'); ?>">
                            <?php echo e($trans['pemasukan'] > 0 ? format_rupiah($trans['pemasukan']) : '-'); ?>

                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm <?php echo e($trans['pengeluaran'] > 0 ? 'text-red-600 font-semibold' : 'text-gray-400'); ?>">
                            <?php echo e($trans['pengeluaran'] > 0 ? format_rupiah($trans['pengeluaran']) : '-'); ?>

                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-blue-600 font-semibold">
                            <?php echo e(format_rupiah($trans['saldo'])); ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const groupBy = <?php echo json_encode($groupBy, 15, 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sriwijaya_kidss\resources\views/laporan/cashflow.blade.php ENDPATH**/ ?>