...
@section('title', 'Edit Kelas - ' . config('app.name'))
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-lg shadow-slate-200 overflow-hidden">
        ...
        <div class="px-8 py-6 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
            <h3 class="text-2xl font-bold text-slate-800">Form Edit Kelas</h3>
            <p class="mt-1 text-sm text-slate-500">Perbarui informasi kelas di bawah ini</p>
        </div>

        <form action="{{ route('kelas.update', $kelas->id ?? 1) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="tingkat" 
                        ...
                        <option value="7" {{ old('tingkat', $kelas->tingkat ?? '') == 7 ? 'selected' : '' }}>Tingkat 7 (SMP Kelas 1)</option>
                        <option value="8" {{ old('tingkat', $kelas->tingkat ?? '') == 8 ? 'selected' : '' }}>Tingkat 8 (SMP Kelas 2)</option>
                        <option value="9" {{ old('tingkat', $kelas->tingkat ?? '') == 9 ? 'selected' : '' }}>Tingkat 9 (SMP Kelas 3)</option>
                        <option value="10" {{ old('tingkat', $kelas->tingkat ?? '') == 10 ? 'selected' : '' }}>Tingkat 10 (SMA Kelas 1)</option>
                        <option value="11" {{ old('tingkat', $kelas->tingkat ?? '') == 11 ? 'selected' : '' }}>Tingkat 11 (SMA Kelas 2)</option>
                        <option value="12" {{ old('tingkat', $kelas->tingkat ?? '') == 12 ? 'selected' : '' }}>Tingkat 12 (SMA Kelas 3)</option>
                    </select>
                    @error('tingkat')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
