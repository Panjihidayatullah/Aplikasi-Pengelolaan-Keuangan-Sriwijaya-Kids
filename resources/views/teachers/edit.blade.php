...
@section('title', 'Edit Guru - ' . config('app.name'))
...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Perbarui profil atau manajemen akun guru.</p>
        </div>
    </div>
    ...
    <form action="{{ route('guru.update', $guru->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-8 space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir', optional($guru->tanggal_lahir)->format('Y-m-d')) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat" id="alamat" rows="3"
                            class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">{{ old('alamat', $guru->alamat) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="pendidikan_terakhir" id="pendidikan_terakhir" value="{{ old('pendidikan_terakhir', $guru->pendidikan_terakhir) }}" placeholder="Contoh: S1 Matematika, S2 Pendidikan Islam, dll"
                            class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="foto" class="block text-sm font-medium text-gray-700 mb-1">Ubah Foto Profil</label>
                        ...
                        <p class="mt-1 text-xs text-gray-500">Abaikan jika tidak ingin mengubah foto. Format: JPG, JPEG, PNG. Maksimal 2MB.</p>
                        
                        @if($guru->foto)
                            <div class="mt-3">
                                <span class="block text-xs font-medium text-gray-500 mb-1">Foto saat ini:</span>
                                <img src="{{ Storage::url($guru->foto) }}" alt="Foto Guru" class="h-20 w-20 rounded-xl object-cover border">
                            </div>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $guru->is_active) ? 'checked' : '' }}>
            ...
            <!-- Akun Login -->
            <div class="bg-blue-50/50 rounded-2xl p-6 border border-blue-100" x-data="{ buatAkun: {{ old('buat_akun_login') ? 'true' : 'false' }} }">
                <div class="flex items-start gap-4 mb-4">
                    ...
                    <div>
                        <h3 class="text-base font-semibold text-gray-900">
                            {{ $guru->user_id ? 'Perbarui Password / Akun Login?' : 'Buatkan Akun Login?' }}
                        </h3>
                        <p class="text-sm text-gray-600">
                            {{ $guru->user_id ? 'Centang opsi ini jika Anda ingin mengganti password akun login guru tersebut atau menyinkronkan data.' : 'Aktifkan untuk membuatkan akun bagi guru ini agar dapat login ke dashboard.' }}
                        </p>
                    </div>
                </div>
