...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-lg shadow-slate-200 overflow-hidden">
        ...
        <div class="px-8 py-6 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
            <h3 class="text-2xl font-bold text-slate-800">Form Edit Pembayaran</h3>
            <p class="mt-1 text-sm text-slate-500">Perbarui data pembayaran siswa</p>
        </div>

        <form action="{{ route('pembayaran.update', $pembayaran->id) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="metode_pembayaran" 
                        ...
                        <option value="tunai" {{ old('metode_pembayaran', strtolower($pembayaran->metode_bayar)) == 'tunai' ? 'selected' : '' }}>Tunai</option>
                        <option value="transfer" {{ old('metode_pembayaran', strtolower($pembayaran->metode_bayar)) == 'transfer' ? 'selected' : '' }}>Transfer Bank</option>
                        <option value="qris" {{ old('metode_pembayaran', strtolower($pembayaran->metode_bayar)) == 'qris' ? 'selected' : '' }}>QRIS</option>
                    </select>
