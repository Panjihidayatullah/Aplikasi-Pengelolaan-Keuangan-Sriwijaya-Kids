...
<div class="max-w-5xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg shadow-slate-200 overflow-hidden">
                <div class="px-6 py-5 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                    <h4 class="text-lg font-bold text-slate-800">Informasi Pembayaran</h4>
                </div>
                <div class="p-8 space-y-6">
                    <div class="flex items-start py-3">
                        ...
                        <div class="w-3/5">
                            <p class="text-base font-bold text-blue-600">{{ $pembayaran->kode_transaksi }}</p>
                        </div>
                    </div>

                    <div class="flex items-start py-3 border-t border-slate-100">
                        <div class="w-2/5">
                            <p class="text-sm font-semibold text-slate-500">Jenis Pembayaran</p>
                        </div>
                        <div class="w-3/5">
                            <p class="text-base font-semibold text-slate-800">{{ $pembayaran->jenis->nama ?? '-' }}</p>
                            @if($pembayaran->jenis->keterangan)
                                <p class="text-sm text-slate-500 mt-2">{{ $pembayaran->jenis->keterangan }}</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-start py-3 border-t border-slate-100">
                        <div class="w-2/5">
                            <p class="text-sm font-semibold text-slate-500">Jumlah Bayar</p>
                        </div>
                        <div class="w-3/5">
                            <p class="text-3xl font-bold text-green-600">Rp {{ number_format($pembayaran->jumlah, 0, ',', '.') }}</p>
                        </div>
                    </div>

                    <div class="flex items-start py-3 border-t border-slate-100">
                        <div class="w-2/5">
                            <p class="text-sm font-semibold text-slate-500">Tanggal Bayar</p>
                        </div>
                        <div class="w-3/5">
                            <p class="text-base text-slate-800">{{ $pembayaran->tanggal_bayar->format('d F Y') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start py-3 border-t border-slate-100">
                        ...
                        <div class="w-3/5">
                            <p class="text-base text-slate-700">{{ $pembayaran->keterangan }}</p>
                        </div>
                    </div>
                    @endif
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg shadow-slate-200 overflow-hidden">
                <div class="px-6 py-5 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                    <h4 class="text-lg font-bold text-slate-800">Data Siswa</h4>
                </div>
                <div class="p-8 space-y-6">
                    <div class="flex items-start py-3">
                        <div class="w-2/5">
                        ...
                        <div class="w-3/5">
                            <p class="text-base font-semibold text-slate-800">{{ $pembayaran->siswa->nis ?? '-' }}</p>
                        </div>
                    </div>

                    <div class="flex items-start py-3 border-t border-slate-100">
                        <div class="w-2/5">
                            <p class="text-sm font-semibold text-slate-500">Nama Lengkap</p>
                        </div>
                        <div class="w-3/5">
                            <p class="text-base font-semibold text-slate-800">{{ $pembayaran->siswa->nama ?? '-' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start py-3 border-t border-slate-100">
                        ...
                        <div class="w-3/5">
                            <p class="text-base text-slate-800">{{ $pembayaran->siswa->kelas->nama_kelas ?? '-' }}</p>
                        </div>
                    </div>

                    @if($pembayaran->siswa->telepon)
                    <div class="flex items-start py-3 border-t border-slate-100">
                        <div class="w-2/5">
                            <p class="text-sm font-semibold text-slate-500">Telepon</p>
                        </div>
                        <div class="w-3/5">
                            <p class="text-base text-slate-800">{{ $pembayaran->siswa->telepon }}</p>
                        </div>
                    </div>
                    @endif
                </div>
        ...
        <div class="lg:col-span-1 space-y-8 no-print">
            ...
            <!-- Actions -->
            <div class="bg-white rounded-2xl shadow-lg shadow-slate-200 overflow-hidden p-6 space-y-3">
                <a href="{{ route('pembayaran.index') }}" 
                   class="w-full px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl transition-all duration-200 flex items-center justify-center space-x-2">
                ...
                <form action="{{ route('pembayaran.destroy', $pembayaran->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pembayaran ini?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
