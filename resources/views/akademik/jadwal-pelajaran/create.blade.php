...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Admin mengatur jadwal kelas, mapel, guru, hari, dan jam.</p>
    </div>

    <div class="bg-white rounded-lg shadow p-8 md:p-10 max-w-6xl w-full mx-auto">
        <form method="POST" action="{{ route('akademik.jadwal-pelajaran.store') }}" class="space-y-6">
            @csrf

            <div class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
                <label class="inline-flex items-center gap-2 text-sm font-semibold text-amber-900">
                    <input id="is_istirahat" type="checkbox" name="is_istirahat" value="1" @checked(old('is_istirahat'))>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="time" name="jam_mulai" value="{{ old('jam_mulai', '07:00') }}" class="w-full px-4 py-2 border @error('jam_mulai') border-red-400 @else border-gray-300 @enderror rounded-lg" required>
                ...
                <div>
                    ...
                    <input type="time" name="jam_selesai" value="{{ old('jam_selesai', '08:00') }}" class="w-full px-4 py-2 border @error('jam_selesai') border-red-400 @else border-gray-300 @enderror rounded-lg" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="ruang_id" name="ruang_id" class="w-full px-4 py-2 border @error('ruang_id') border-red-400 @else border-gray-300 @enderror rounded-lg" required>
                        ...
                        <option value="{{ $ruang->id }}" @selected(old('ruang_id') == $ruang->id)>{{ $ruang->kode_ruang }} - {{ $ruang->nama }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3" class="w-full px-4 py-2 border @error('keterangan') border-red-400 @else border-gray-300 @enderror rounded-lg" placeholder="Catatan tambahan...">{{ old('keterangan') }}</textarea>
                @error('keterangan')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="flex gap-3 pt-2">
                <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold transition">Simpan Jadwal</button>
