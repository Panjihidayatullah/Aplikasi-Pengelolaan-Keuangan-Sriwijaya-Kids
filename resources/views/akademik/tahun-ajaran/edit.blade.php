...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Perbarui informasi tahun akademik</p>
    </div>

    <!-- Form -->
    <div class="max-w-6xl w-full mx-auto">
        <div class="bg-white rounded-lg shadow-md p-8 md:p-10">
            <form method="POST" action="{{ route('akademik.tahun-ajaran.update', $tahunAjaran) }}">
                @csrf @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <select id="kurikulum_id" name="kurikulum_id" 
                        ...
                        <option value="{{ $k->id }}" {{ old('kurikulum_id', $tahunAjaran->kurikulum_id) == $k->id ? 'selected' : '' }}>
                            {{ $k->nama }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="tahun_selesai" name="tahun_selesai" value="{{ old('tahun_selesai', $tahunAjaran->tahun_selesai) }}" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            required>
                        @error('tahun_selesai')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div>
                        ...
                        <input type="date" id="tanggal_mulai" name="tanggal_mulai" value="{{ old('tanggal_mulai', $tahunAjaran->tanggal_mulai->format('Y-m-d')) }}" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            required>
                        @error('tanggal_mulai')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="date" id="tanggal_selesai" name="tanggal_selesai" value="{{ old('tanggal_selesai', $tahunAjaran->tanggal_selesai->format('Y-m-d')) }}" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            required>
