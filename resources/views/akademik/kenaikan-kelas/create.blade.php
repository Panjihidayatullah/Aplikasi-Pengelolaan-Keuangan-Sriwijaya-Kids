...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Proses Kenaikan Kelas</h1>
            <p class="text-gray-600 mt-1">Buat keputusan kenaikan kelas siswa</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-8 md:p-10 max-w-6xl w-full mx-auto">
        <form method="POST" action="{{ route('akademik.kenaikan-kelas.store') }}">
            @csrf
            ...
            <div class="mb-6">
                ...
                <select id="siswa_id" name="siswa_id" class="w-full px-4 py-2 border @error('siswa_id') border-red-400 @else border-gray-300 @enderror rounded-lg" required>
                    ...
                    <option
                        ...
                        @selected(old('siswa_id') == $siswa->id)
                    >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="mb-6">
                    ...
                    <select id="kelas_sekarang_id" name="kelas_sekarang_id" class="w-full px-4 py-2 border @error('kelas_sekarang_id') border-red-400 @else border-gray-300 @enderror rounded-lg" required>
                        ...
                        @endphp
                        <option value="{{ $kelas->id }}" data-tingkat="{{ $tingkat > 0 ? $tingkat : '' }}" data-rombel="{{ $rombel }}" data-is-lulus="{{ $kelas->is_tingkat_akhir ? '1' : '0' }}" @selected(old('kelas_sekarang_id') == $kelas->id)>{{ $formatKelasLabel($kelas) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-6">
                    ...
                    <select id="kelas_tujuan_id" name="kelas_tujuan_id" class="w-full px-4 py-2 border @error('kelas_tujuan_id') border-red-400 @else border-gray-300 @enderror rounded-lg">
                        ...
                        @endphp
                        <option value="{{ $kelas->id }}" data-tingkat="{{ $tingkat > 0 ? $tingkat : '' }}" data-rombel="{{ $rombel }}" data-is-lulus="{{ $kelas->is_tingkat_akhir ? '1' : '0' }}" @selected(old('kelas_tujuan_id') == $kelas->id)>{{ $formatKelasLabel($kelas) }}</option>
                        @endforeach
                    </select>
                    <p id="kelas_tujuan_hint" class="text-xs text-gray-500 mt-2">Sistem akan merekomendasikan kelas tujuan sesuai tingkat berikutnya dengan rombel yang sama jika tersedia.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="mb-6">
                    ...
                    <input type="number" step="0.01" min="0" max="100" id="rata_rata_nilai" name="rata_rata_nilai" value="{{ old('rata_rata_nilai') }}" class="w-full px-4 py-2 border @error('rata_rata_nilai') border-red-400 @else border-gray-300 @enderror rounded-lg" required>
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="status" name="status" class="w-full px-4 py-2 border @error('status') border-red-400 @else border-gray-300 @enderror rounded-lg" required>
                    <option value="naik" @selected(old('status') == 'naik')>Naik Kelas</option>
                    <option value="tidak_naik" @selected(old('status') == 'tidak_naik')>Tidak Naik (Menetap)</option>
            ...
            <div class="mb-6">
                ...
                <textarea id="catatan" name="catatan" rows="3" class="w-full px-4 py-2 border @error('catatan') border-red-400 @else border-gray-300 @enderror rounded-lg">{{ old('catatan') }}</textarea>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('akademik.kenaikan-kelas.index') }}" class="px-6 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg font-semibold transition">Batal</a>
            </div>
        </form>
    </div>
</div>
...
<script>
    (function () {
        ...
        function setKelasSekarangFromSiswa() {
