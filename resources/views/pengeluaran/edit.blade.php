            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="jenis_pengeluaran_id" 
                        ...
                        @foreach(\App\Models\JenisPengeluaran::orderBy('nama')->get() as $jenis)
                            <option value="{{ $jenis->id }}" {{ old('jenis_pengeluaran_id', $pengeluaran->jenis_pengeluaran_id) == $jenis->id ? 'selected' : '' }}>
                                {{ $jenis->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" 
                              ...
                              class="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all duration-200 resize-none @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $pengeluaran->deskripsi) }}</textarea>
                ...
                <div>
                    ...
                    <select name="status" 
                        ...
                        <option value="approved" {{ old('status', $pengeluaran->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                        <option value="pending" {{ old('status', $pengeluaran->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="rejected" {{ old('status', $pengeluaran->status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
                    </select>
                    @error('status')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
